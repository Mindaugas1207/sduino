...
function createFileUploader(d, f, g) {
    ...
    i.onchange = function (e) {
        ...
        if (j.value === "/" || j.value.lastIndexOf("/") === 0) { j.value = "/" + a }
        else { j.value = j.value.substring(0, j.value.lastIndexOf("/") + 1) + a }
    }
}
...
function createTree(k, l) {
    ...
    function fillFileMenu(a, b) {
        var c = document.createElement("ul"); a.appendChild(c); var d = document.createElement("li"); c.appendChild(d);
        if (isTextFile(b)) {
            d.innerHTML = "<span>Edit</span>"; d.onclick = function (e) {
                l.loadUrl(b);
                if (document.body.getElementsByClassName('cm').length > 0) document.body.removeChild(a)
            }
        }
        else if (isImageFile(b)) {
            d.innerHTML = "<span>Preview</span>"; d.onclick = function (e) {
                loadPreview(b);
                if (document.body.getElementsByClassName('cm').length > 0) document.body.removeChild(a)
            }
        }
...
function createEditor(e, f, g, h, i) {
    function getLangFromFilename(a) {
        ...
        if (typeof c !== undefined) {
            switch (c) {
                case "txt": b = "plain"; break;
                case "htm": b = "html"; break;
                case "js": b = "javascript"; break;
                case "c": b = "c_cpp"; break;
                case "cpp": b = "c_cpp"; break;
                case "css":
                case "scss":
                case "php":
                case "html":
    ...
    if (typeof g === "undefined") { g = getLangFromFilename(f) }
    ...
    function httpPost(a, b, c) {
        ...
        var d = new FormData();
        d.append("data", new Blob([b], { type: c }), a);
        j.open("POST", "/edit");
        j.send(d)
    }
...
function onBodyLoad() {
    var c = {};
    var d = window.location.href.replace(/[?&]+([^=&]+)=([^&]*)/gi,
        function (m, a, b) {
            c[a] = b
        }
    );
