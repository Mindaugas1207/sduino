...
static inline int IS31FL3218_reset(IS31FL3218_inst_t *inst)
{
    ...
    if (port_write(&inst->port_device, buff, sizeof(buff)) == PORT_ERROR)
        return IS31FL3218_ERROR;
    return IS31FL3218_OK;
}

static int IS31FL3218_init(IS31FL3218_inst_t *inst)
{
    inst->data.reg = IS31FL3218_PWM_1_REG;
    for (int i = 0; i < IS31FL3218_CHANNEL_COUNT; i++)
        inst->data.pwm[i] = 0;
    inst->data.ctrl[0] = 0;
    inst->data.ctrl[1] = 0;
    inst->data.ctrl[2] = 0;
    ...
    if (IS31FL3218_reset(inst) == IS31FL3218_ERROR)
        return IS31FL3218_ERROR;
    return IS31FL3218_OK;
}
...
{
    if (port_write(&inst->port_device, (uint8_t *)&inst->data, sizeof(inst->data)) == PORT_ERROR)
        return IS31FL3218_ERROR;
    return IS31FL3218_OK;
}
...
static inline int IS31FL3218_write_channels_enable(IS31FL3218_inst_t *inst, uint32_t mask)
{
    inst->data.ctrl[0] = mask;
    inst->data.ctrl[1] = mask >> 6;
    inst->data.ctrl[2] = mask >> 12;
    uint8_t buff[] = {IS31FL3218_LED_CONTROL_1_REG, inst->data.ctrl[0], inst->data.ctrl[1], inst->data.ctrl[2], 0};
    if (port_write(&inst->port_device, buff, sizeof(buff)) == PORT_ERROR)
        return IS31FL3218_ERROR;
    return IS31FL3218_OK;
}
