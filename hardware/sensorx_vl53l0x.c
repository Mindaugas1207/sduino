...
int SENSORX_VL53L0X_Init(VL53L0X_Dev_t *device)
{
	VL53L0X_Error Status = VL53L0X_ERROR_NONE;
	//device->port_device = port_device;
	device->device_enum = 0;
...
int SENSORX_VL53L0X_PollInterrupt(VL53L0X_Dev_t *device)
{
	VL53L0X_Error Status = VL53L0X_ERROR_NONE;
	uint32_t IntStatus;
...
int SENSORX_VL53L0X_PollMeasurementStop(VL53L0X_Dev_t *device)
{
	VL53L0X_Error Status = VL53L0X_ERROR_NONE;
	uint32_t StopStatus;
...
int SENSORX_VL53L0X_RunCalibrations(VL53L0X_Dev_t *device, VL53L0X_Config_t *config, uint32_t calibrationNumber)
{
	VL53L0X_Error Status = VL53L0X_ERROR_NONE;
	...
	switch (calibrationNumber)
	{
		...
		sleep_ms(VL53L0X_OFFSET_CALIBRATION_DELAY_S * 1000);
		...
		sleep_ms(VL53L0X_XTALK_CALIBRATION_DELAY_S * 1000);
...
int SENSORX_VL53L0X_PrintAll(VL53L0X_Dev_t *device, VL53L0X_Config_t *config)
{
	...
	Status |= SENSORX_VL53L0X_GetConfig(device, config);
	...
	printf(
		...
		, config->INTERRUPT.POLARITY ? "HIGH" : "LOW"
